/* Crie três vetores, chamados vetorInteiro, vetorString e vetorDouble. Cada um destes vetores deverá conter
quatro valores, sendo o primeiro com valores inteiros, o segundo com strings e o terceiro com valores decimais.
Declarados os vetores, utilize a função de união concat() para unir os vetores, e mostre o resultado no console. 
Todos os elementos do vetor resultado deverão aparecer no console. */

let vetorInteiro = [1, 2, 3, 4]
let vetorString = [ 'Um', 'Dois', 'Três', 'Quatro']
let vetorDouble = [1.1, 2.2, 3.3, 4.4]
let resultado1 = []
let resultado2 = []

resultado1 = vetorInteiro.concat(vetorString)
resultado2 = vetorInteiro.concat(vetorDouble)

console.log(resultado1)
console.log(resultado2)